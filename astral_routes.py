...
astral_bp = Blueprint("astral_bp", __name__, url_prefix="/astral")
...
def day_pillar_note(day_gz: str):
    ...
    return table.get(day_gz, "日柱：重视自我成长与边界，守正以达成，取长补短为要。")
...
@astral_bp.route("/", methods=["GET"])
def astral_index():
    return "5XLiving Astral API is running."

@astral_bp.route("/health", methods=["GET"])
def astral_health():
    return jsonify({"ok": True, "module": "astral"})

@astral_bp.route("/bazi", methods=["POST"])
def bazi():
